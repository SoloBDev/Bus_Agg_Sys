...
export default function BusRouteManagement() {
  ...
  const fetchRoutes = async () => {
    try {
      ...
      const data = await busAPI.getRoutes(operatorId)

      // Convert backend routes to frontend format
      const formattedRoutes: Route[] = (data.routes || data || []).map((route: any, index: number) => {
        ...
        return {
          ...
          from: route.from,
          to: route.to,
          status: route.status || calculateStatus(route.departureTime, route.arrivalTime),
          selected: index === 0,
  ...
  const updateRouteStatuses = () => {
    setRoutes((prevRoutes) => {
      return prevRoutes.map((route) => {
        const newStatus = calculateStatus(route._rawDepartureTime || "", route._rawArrivalTime || "")

        if (newStatus !== route.status) {
          console.log(`Route ${route._id} status changed from ${route.status} to ${newStatus}`)
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-[#121212] text-white">
        ...
        <button onClick={fetchRoutes} className="px-4 py-2 bg-[#e9d758] text-black rounded-md hover:bg-[#d0c14a]">
          Retry
        </button>
      </div>
    )
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {selectedRoute && <RouteDetails routeDetails={selectedRoute} />}
      </div>
