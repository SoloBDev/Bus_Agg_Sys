...
export default function TransactionHistoryPage() {
  ...
  const fetchTransactions = (url = "http://localhost:3002/api/bookings/transactions"
  ...
  const fetchRouteName = async (ref_id: string) => {
    ...
    try {
      ...
      // }
      // console.log({ref_id})
      const bookingRes = await axios.get(`http://localhost:3002/api/bookings/reference/${ref_id}`,
        {
          headers: {
            "Authorization": `Bearer ${localStorage.getItem("token")}`,
          },
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-left text-sm text-white table-fixed">
          <colgroup>
            ...
            <col className="w-[160px]" /> {/* ROUTE NAME */}
            <col className="w-[110px]" /> {/* AMOUNT */}
            <col className="w-[140px]" /> {/* PAYMENT METHOD */}
            <col className="w-[180px]" /> {/* CHAPA REFERENCE */}
            <col className="w-[200px]" /> {/* MERCHANT REFERENCE */}
          ...
          <thead>
            <tr className="bg-[#181C23] border-b border-[#23272F]">
              ...
              <th className="px-4 py-3 font-semibold text-left">ROUTE NAME</th>
              <th className="px-4 py-3 font-semibold text-left">AMOUNT</th>
              <th className="px-4 py-3 font-semibold text-left">PAYMENT METHOD</th>
              <th className="px-4 py-3 font-semibold text-left">CHAPA REFERENCE</th>
              <th className="px-4 py-3 font-semibold text-left">MERCHANT REFERENCE</th>
              ...
              transactions.map((tx, idx) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    {(tx.first_name || "") + (tx.last_name ? " " + tx.last_name : "") || "N/A"}
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    {routeNames[tx.ref_id] || <span className="text-gray-500">Loading...</span>}
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    {tx.currency || "ETB"} {Number(tx.amount).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </td>
                  {/* ...rest of your columns... */}
                  <td className="px-4 py-3 whitespace-nowrap">{tx.payment_method || "N/A"}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      <span className="truncate max-w-[140px] block">{tx.ref_id}</span>
                      <button
                        className="ml-1"
                        onClick={e => { e.stopPropagation(); navigator.clipboard.writeText(tx.ref_id); }}
                        title="Copy"
                      >
                        <svg width="14" height="14" fill="currentColor" className="inline text-gray-400 hover:text-white"><path d="M3 3v10h10V3H3zm9 9H4V4h8v8zm-1-7H5v6h6V5z"/></svg>
                      </button>
                    </div>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      <span className="truncate max-w-[160px] block">{tx.merchant_reference || "N/A"}</span>
                      <button
                        className="ml-1"
                        onClick={e => { e.stopPropagation(); navigator.clipboard.writeText(tx.merchant_reference || ""); }}
                        title="Copy"
                      >
                        <svg width="14" height="14" fill="currentColor" className="inline text-gray-400 hover:text-white"><path d="M3 3v10h10V3H3zm9 9H4V4h8v8zm-1-7H5v6h6V5z"/></svg>
                      </button>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">{tx.type || "API"}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    {tx.created_at
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
          <div className="bg-[#181C23] rounded-lg shadow-lg w-full max-w-lg p-8 relative">
            ...
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <div className="text-2xl font-bold">
                  {selectedTx.currency || "ETB"} {Number(selectedTx.amount).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                </div>
              </div>
              <span
                className={`px-3 py-1 rounded-full text-xs font-semibold ${
            ...
            <div className="space-y-3">
              <DetailRow label="Customer Name" value={selectedTx.first_name || "N/A"} />
              <DetailRow label="Customer Email" value={selectedTx.email || "N/A"} />
              <DetailRow label="Transaction Reference" value={selectedTx.ref_id} copy />
              <DetailRow label="Payment Processor Reference" value={selectedTx.processor_reference || "-"} />
              <DetailRow label="Channel" value={<span className="inline-flex items-center gap-2">{selectedTx.payment_method ? <span className="bg-yellow-900 text-yellow-300 px-2 py-1 rounded text-xs font-semibold">Test</span> : "-"} </span>} />
              <DetailRow label="Chapa Fees" value={`ETB ${Number(selectedTx.charge || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}`} />
              <DetailRow label="Amount Received" value={`ETB ${(Number(selectedTx.amount || 0) - Number(selectedTx.charge || 0)).toLocaleString(undefined, { minimumFractionDigits: 2 })}`} bold />
              <DetailRow label="Refund Amount" value={`ETB ${Number(selectedTx.refund_amount || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}`} />
              <DetailRow label="Who Paid Fees?" value="You" />
              <DetailRow label="Settled?" value={<span className="bg-red-900 text-red-300 px-3 py-1 rounded-full text-xs font-semibold">NO</span>} />
            </div>
          </div>
        </div>
      )}
...
function DetailRow({ label, value, copy = false, bold = false }: { label: string; value: any; copy?: boolean; bold?: boolean }) {
  return (
    <div className="flex items-center gap-4 border-b border-[#23272F] py-2">
      <div className="w-1/3 text-gray-400">{label}</div>
      <div className={`flex-1 flex items-center gap-2 ${bold ? "font-bold text-lg" : ""}`}>
